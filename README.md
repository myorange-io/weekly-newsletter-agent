...
예제 두 개가 들어 있습니다. 인터넷 없이도 60초면 문지기가 빈 게시판을 막아내는 걸 봅니다.
...
이 세 가지는 프롬프트를 길게 쓴다고 해결되지 않습니다. **일을 쪼개고 쪼갠 자리마다 다른 판단 기준을 두어야** 풀립니다.
이 저장소에는 그 경계를 어디에 그었는지 적었습니다.
...
**아홉 중 AI가 판단하는 건 셋뿐입니다.** 나머지는 코드가 합니다 — 일부러 그렇게 했습니다.
기계로 끝나는 판정을 AI에게 맡기면 열 번에 한두 번 틀리고 왜 틀렸는지도 모릅니다.
...
"AI가 지어내지 않고 우리 말투로 쓰는" 상태가 됩니다. 나머지는 필요해질 때 붙이면 됩니다.
...
`voice-rules.md`에는 **오렌지레터가 제보 459건의 편집 전후 쌍에서 뽑아낸 규칙 15개**가 그대로 들어 있습니다
(예시 문장은 전부 가상으로 다시 썼습니다). **베끼라고 넣은 게 아니라 모양을 보라고 넣었습니다** —
`"A를 만나면 B로 바꾼다" + 왜 그런지 한 줄` 형태가 열다섯 개 중 열넷입니다.
"간결하게 쓰세요" 같은 서술보다 이 형태가 훨씬 잘 먹힙니다.
...
- **ChatGPT · Claude 웹만 쓴다면** — `.claude/agents/*.md`의 절차와 '지킬 것'을 그대로 프롬프트에 붙입니다.
  **한 번에 한 명씩** 부르는 게 핵심입니다. 초안가와 비평가는 **반드시 대화창을 새로 열어야** 합니다.
- **다른 언어로 옮긴다면** — `scripts/`는 하나같이 의존성 없는 200줄 안팎짜리입니다. 판단 기준이 곧 분기 조건입니다.
...
**[PORTING.md](PORTING.md)** — 일을 어디서 잘라야 하는지 정리한 규칙 넷입니다.
...
실제 구현에는 조직 고유의 데이터·인증·운영 설정이 얽혀 있어 그대로 공개하지 않았습니다.
여기 있는 건 같은 판단 기준을 의존성 없이 다시 짠 물건입니다. 특히 이런 게 빠져 있습니다.

- **본문 추출이 단순합니다.** 정규식으로 태그를 걷어내는 수준이라, 사이트별 전용 추출 경로가 없습니다
- **채널 인증·API 연동이 없습니다.** "채널이 받을 수 있는 형태"까지만 만듭니다
- **문체 규칙의 예시 문장이 가상입니다.** 규칙 열다섯 개는 실물 그대로지만 예시에 딸려 나오는 실제 제보 문장은 조직 안에 뒀습니다

쓰다가 막히는 곳이 있거나 자기 조직에 옮겨보셨다면 이슈로 남겨주세요.
